...
#define VREG_VDD	"8226_l19"
#define VREG_VBUS	"8226_lvs1"
...
static int touchscreen_gpio_init(int flag,char *vreg_vdd,char *vreg_vbus,
...
void zte_synaptics_change_fw_config( struct synaptics_rmi4_data *ts )//xym
{
	...
	uint8_t ecw_data0[1]={0x07};//F11_2D_CTRL08
	uint8_t ecw_data1[1]={0x00};//F11_2D_CTRL47
	...
	if(ts->config_id.sensor==0x39)//ECW
	{
		...
		retval=synaptics_rmi4_i2c_write(ts,0x5f,ecw_data0,1); 
		if (retval < 0) dev_err(&(ts->input_dev->dev), "%s: Failed to write 5f\n", __func__);

		retval=synaptics_rmi4_i2c_write(ts,0x78,ecw_data1,1); 
		if (retval < 0) dev_err(&(ts->input_dev->dev), "%s: Failed to write 78\n", __func__);

		retval=synaptics_rmi4_i2c_write(ts,0x79,ecw_data2,1); 
		if (retval < 0) dev_err(&(ts->input_dev->dev), "%s: Failed to write 79\n", __func__);

		i2c_smbus_write_byte_data(ts->i2c_client,0xff,3);// change to page 3.
		retval=synaptics_rmi4_i2c_write(ts,0x305,ecw_data3,7); 
		if (retval < 0) dev_err(&(ts->input_dev->dev), "%s: Failed to write 305\n", __func__);
...
*/
	...
	retval=touchscreen_gpio_init(1,platform_data->vdd,platform_data->vbus,
...
err_detect:
	touchscreen_power(0);
	touchscreen_gpio_init(0,platform_data->vdd,platform_data->vbus,
		platform_data->reset_gpio,platform_data->irq_gpio);
